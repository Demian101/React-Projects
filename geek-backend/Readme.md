...
![](http://imagesoda.oss-cn-beijing.aliyuncs.com/Sodaoo/2022-06-27-105848.png)
...
> **懒加载**通过对组件进行分割打包成多个chunk来减少一次性加载的资源大小。从而减少用户不必要的等待。
>
> 加载首页的时候并不需要加载其他业务模块，因此这些业务模块对应的组件都可以通过懒加载的形式来引入，加快首屏渲染速度，提高用户转化率。

```react
// 比如下面这些 Page ,首页的时候用不到, 就不用渲染了, 用到的时候再渲染.
...
蚂蚁金服开发的 React 组件库：`yarn add antd`
...
`本节目标:`  能够让 vscode 识别 @ 路径并给出路径提示

( 除了让 webpack 知道, 也需要让 VSCode 知道 , 并进行代码编写提示 ) 
...
> 比如说你 没登录 , 就想访问  /articles  , 是不允许的, 得先给我登录去
...
`本节目标:`  能够在响应拦截器中处理 token 失效

> 当 token 失效后, 需要跳转到登录 login 页面去重新登录获取 Token 
>
> 但是react 的 Router 不支持在组件之外完成路由跳转， 所以用 history 来实现, 下图是一个社区的示例
